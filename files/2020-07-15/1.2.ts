...
// 必选参数必须在可选参数和默认参数之前
function optional1(a?: number, b: number = 100, c: number): number {
	return a ? a + b : b * b;
}
...
const ov_a = overload('numberToString', 100); // ts 自动推断出类型为 string
const ov_b = overload('stringToNumber', '100'); // ts 自动推断出类型为 number
const ov_c = overload('toBoolean', '100'); // ts 自动推断出类型为 boolean
const ov_d = overload('toBoolean', 100); // ts 自动推断出类型为 boolean
