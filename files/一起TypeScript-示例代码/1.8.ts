...
function getName(data: IUserData): IUserData['name'] {
	return data.name;
}
...
function getContact2(data: IUserData): typeof data['mobile'] {
	return data.mobile;
}

function getContact3(data: IUserData): (typeof data)['mobile'] {
	return data.mobile;
}
...
// keyof 操作符之后必须是类型，如果想要直接从对象上获取属性名集合，要借助 typeof
let userData = {
	name: 'foo',
	age: 12,
};
...
const car: Module<{
	...
	methods: { // methods 内部上下文的 this 被指向了新类型 D & M
		move() {
			this.x++;
			this.getPos();
			this.y++;
